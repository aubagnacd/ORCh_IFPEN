...
for i in range(ntimesteps):
	...
	for j in range(nparts):
		...
		Yref[i,j,:] = local_df.loc[local_df["Particle_number"] == j,species_to_plot].values
		YPAHref[i,j,:] = local_df.loc[local_df["Particle_number"] == j,PAH_to_plot].values
		local_part.T = Tref[i,j]
		local_part.calcZ(gas)
		Zref[i,j] = local_part.Z
		#
		# Fill Zlist, Zind, Ymax and YPAHmax
		#
		if (local_part.Z not in Zlist):
			Zlist = np.append(Zlist,local_part.Z)
			Ymax = np.concatenate((Ymax,np.zeros((1,n2plot))))
			YPAHmax = np.concatenate((YPAHmax,np.zeros((1,nPAH2plot))))
		locZind = np.where(Zlist == local_part.Z)
...
for ired in range(mech_range):
	...
	if (os.path.exists(filename)):
		...
		for i in range(ntimesteps):
			...
			for j in range(nparts):
				local_part.Yk = local_df.loc[local_df["Particle_number"] == j,gas.species_names].values
				Y[i,j,:] = local_df.loc[local_df["Particle_number"] == j,species_to_plot].values
				YPAH[i,j,:] = local_df.loc[local_df["Particle_number"] == j,PAH_to_plot].values
				local_part.T = Tref[i,j]
				local_part.calcZ(gas)
				Z[i,j] = local_part.Z
				#
				errY[i,j,:] = np.divide(np.abs(Y[i,j,:]-Yref[i,j,:]), Yref[i,j,:], out=np.zeros_like(Y[i,j,:]), where=((Yref[i,j,:] > eps) & (Y[i,j,:] > Ymax[Zind[i,j],:]*1.0e-2)))
				errYPAH[i,j,:] = np.divide(np.abs(YPAH[i,j,:]-YPAHref[i,j,:]), YPAHref[i,j,:], out=np.zeros_like(YPAH[i,j,:]), where=(local_part.Z > Zcut) & (local_part.T > Tcut) & (YPAHref[i,j,:] > eps) & (YPAH[i,j,:] > YPAHmax[Zind[i,j],:]*1.0e-2))
